...
$bins = array('BOLD:AAA9566'); // DOI http://dx.doi.org/10.5883/BOLD:AAA9566
...
//{
	...
	if ($html == '')
	{
		// Oops, no HTML, make blank entry we can check later
		// BIN
		$sql = 'REPLACE INTO bins(bin) VALUES ("' . $bin . '");';
		echo $sql . "\n";	
	}
	else
	{
			...
			foreach ($tables as $table)
			{
				...
				foreach ($trs as $tr)
				{
					...
					*/
					...
					foreach ($tds as $td)
					{
						if (preg_match('/http:\/\/dx.doi.org\/10.5883\/' . $bin . '/', $td->plaintext, $m))
						{
							$taxonomy['doi'][] = '10.5883/' . $bin;
						}
					}				
